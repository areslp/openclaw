...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-6-astra` for Codex OAuth access). If you omit the provider, OpenClaw tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, OpenClaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- Model `params.thinking` sets that model's thinking default; an agent-specific model entry overrides the shared entry. Per-message and session choices, followed by the agent's `thinkingDefault`, take precedence over either model setting. See [thinking resolution order](/tools/thinking#resolution-order).
...
- `compat.thinkingFormat`: OpenAI-compatible thinking payload style. Use `"together"` for Together-style `reasoning.enabled`, `"qwen"` for Qwen-style top-level `enable_thinking`, or `"qwen-chat-template"` for `chat_template_kwargs.enable_thinking` on Qwen-family backends that support request-level chat-template kwargs, such as vLLM. OpenClaw maps disabled thinking to `false` and enabled thinking to `true`. With `"qwen-chat-template"`, enabled levels also send `chat_template_kwargs.reasoning_effort` using the Qwen 3.8 tiers: `minimal` and `low` send `low`, `medium` sends `medium`, and `high`, `xhigh`, and `max` send `xhigh`, the template default; templates without that variable ignore it. Without a configured thinking level, these models default to `high`, which keeps the template's `xhigh` depth. Configured vLLM Qwen models expose binary `/think` choices for these formats and leave the effort at the template default.
...
- `maxConcurrent`: max parallel agent runs across sessions (each session still serialized). By default, OpenClaw uses `max(8, available CPU parallelism * 4)`, based on `os.availableParallelism()` with `os.cpus().length` as a fallback. This allows 32 runs on 8 available CPUs and 192 on 48. An explicit value overrides the CPU-derived default, including after upgrades.
